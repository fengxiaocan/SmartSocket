...
import com.smart.socket.server.impl.iocore.ClientIOManager;
...
public class ClientImpl extends AbsClient {
    ...
    @Override
    public IClient send(ISendPack sender) {
        if (mIOManager != null) {
            mIOManager.send(sender);
        }
        return this;
    }
    ...
    @Override
    public void onClientWrite(ISendPack sendable) {
        List<IClientIOCallback> list = new ArrayList<>();
        list.addAll(mCallbackList);
